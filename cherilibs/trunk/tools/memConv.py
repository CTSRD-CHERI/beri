...
def writeMIF(memArray, memSize, prefix, width):
	bytes = width/8;
	numDWords = memSize / bytes;
	
	if(memSize % bytes):
		lastDWordMaxByte = memSize % bytes
		numDWords = numDWords + 1
	else:
		lastDWordMaxByte = bytes
	
	memMIF = open(prefix + str(width) + ".mif", "w")
	
	addr = 0
	
	memMIF.write("-- generated by memConv.py\n");
	memMIF.write("DEPTH = %d;\n" % (numDWords));
	memMIF.write("WIDTH = %d;\n" % (bytes*8));
	memMIF.write("ADDRESS_RADIX = HEX;\n");
	memMIF.write("DATA_RADIX = HEX;\n");
	memMIF.write("CONTENT\n");
	memMIF.write("BEGIN\n");

	format_string = "%08X:%0"+str(bytes*2)+"X;\n"
	print format_string
	...
	for i in range(0, numDWords):
		...
		for j in range(0, bytes):
			...
			value += memArray[i * bytes + j] * mult
			mult *= 256L
			
		memMIF.write(format_string % (addr, value))
		addr += 1
...
if __name__=="__main__":
	...
	for arg in args:
		if(arg == "bsim"):
			...
			writeMIF(memArray, memSize, opts.verilog_prefix, 32)
			writeMIF(memArray, memSize, opts.verilog_prefix, 64)
			writeMIF(memArray, memSize, opts.verilog_prefix, 256)
		else:
			print "Unrecognised output mode: %s. Use bsim, bsimc2, mif or verilog." % arg
