...
///////////////////////////////
int go = 0; // used for synchronizing cores before first semaphore gets initialized
...
int end = 0; // control the end of the main loop
...
int main ()
{
    ...
    while (!end)
    {
        ...
        if (core_counter_res())
            total_cycles = (counter_value_end - counter_value_start)/core_counter_res();
