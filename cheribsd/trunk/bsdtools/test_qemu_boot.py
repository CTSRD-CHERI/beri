...
#                       be run
#   CHERIABITEST_LIST_XML: name of XML file containing list of cheritest tests
#                       to be run
#
...
# The whole lot takes <10 seconds on 2016-01-27
boot_timeout = 60 # hard timeout for boot test
hw_timeout = 60 # hard timeout for hello world test
cheritest_timeout = 60 # hard timeout for entire cheritest test
post_error_timeout = 60 # how long to wait (after an error has been
...
def setup_module():
  ...
  global sim
  qemu_cmd = os.getenv("QEMU_CMD", "qemu-system-cheri")
  kernel = os.getenv("QEMU_KERNEL", "./kernel")
  diskimage = os.getenv("QEMU_DISKIMAGE", "./diskimage")
  #sim = subprocess.Popen(["make",target], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
  sim = subprocess.Popen([qemu_cmd,
                          "-M", "malta", "-kernel", kernel,
                          "-hda", diskimage, "-m", "2048", "-nographic"],
...
# Test whether the simulation has successfully booted CheriBSD.  This
...
# Run cheritest tests. The list of tests is found in the xml file specified
# by the environment variable CHERIABITEST_LIST_XML
#@timed(cheritest_timeout) # timeout of 3 hours
def test_cheriabitest():
  
  write_unbuffered("Running cheriabitest -a -f\n")
  ...
  # to prevent output being lost by the UART
  if (os.getenv("SLEEP_AFTER_TEST") != None):
    sim.stdin.write('/bin/cheriabitest -a -f -s --libxo xml,pretty\n')
  else:
    sim.stdin.write('/bin/cheriabitest -a -f --libxo xml,pretty\n')
  ...
  tree = ET.parse(os.getenv("CHERIABITEST_LIST_XML"))
  root = tree.getroot()
  for test in root.iter('test'):
    if timeout_exceeded:
      return
  ...
  elapsed_secs = time.time()-start
  write_unbuffered("cheriabitest tests finished - took %.2f secs\n" % elapsed_secs)
